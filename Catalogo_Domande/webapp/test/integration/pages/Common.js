sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("cap/catalogo_domande/Catalogo_Domande/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#CatalogoDomande-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#CatalogoDomande-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("cap.catalogo_domande.Catalogo_Domande.test.integration.pages.Common", {
		...
		getMockServer: function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["cap/catalogo_domande/Catalogo_Domande/localService/mockserver"], function (mockserver) {
